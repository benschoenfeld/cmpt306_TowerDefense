...
var money_counter_scene: PackedScene = preload("res://general_ui/hud/number_display.tscn")
var money_counter: NumberDisplay = money_counter_scene.instantiate()
...
func test_display_amount():
	...
	assert_eq(money_counter.number_label.text, "0")
	...
	assert_eq(money_counter.number_label.text, "4242")
	
	# Testing past the max value (99999)
	money_counter.display_amount(100000000)
	assert_eq(money_counter.number_label.text, "99999")
